...
def main():
    ...
    for i, (res, sigma) in enumerate(zip(resolutions, sigmas)):
        ret = raycast((phis, thetas), res, latents, sdf, gradScaler, sigma / real.shape[2])['full']
    ...
    for i, (res, sigma) in enumerate(zip(resolutions, sigmas)):
        size = np.prod(res)
        blurred = blur(real, sigma)
        out = torch.nn.functional.interpolate(blurred, size=(size, size), mode='bilinear')[0]
